...
describe('Chainhook Notification Integration Flow', () => {
  ...
  describe('End-to-End Notification Flow', () => {
    ...
    it('should respect user notification preferences', async () => {
      ...
      const filtered =
        await NotificationService.filterNotificationsByPreferences(
    ...
    it('should store and retrieve notifications in delivery service', async () => {
      ...
      const delivered = await NotificationDeliveryService.deliverNotification(
        notification
      );
      ...
      const userNotifications =
        await NotificationDeliveryService.getUserNotifications(userId);
    ...
    it('should mark notifications as read correctly', async () => {
      ...
      const unreadBefore = await NotificationDeliveryService.getUnreadCount(
        userId
      );

      await NotificationDeliveryService.markNotificationAsRead(
        userId,
        delivered._id
      );

      const unreadAfter = await NotificationDeliveryService.getUnreadCount(
        userId
      );
    ...
    it('should filter delivered notifications by specific type', async () => {
      ...
      const badgeNotifications =
        await NotificationDeliveryService.getNotificationsByType(
          userId,
          'badge_received'
        );

      expect(badgeNotifications.length).toBeGreaterThan(0);
      expect(badgeNotifications.every((n) => n.type === 'badge_received')).toBe(
        true
      );
    });
    ...
    it('should handle user preference updates and re-enabling', async () => {
      ...
      await NotificationService.disableNotificationType(
        userId,
        'badge_received'
      );
      ...
      await NotificationService.enableNotificationType(
        userId,
        'badge_received'
      );
