export const walletConnectMainnetStagingConfig = {
  ...
  relayUrl:
    process.env.WALLETCONNECT_RELAY_URL || 'wss://relay.walletconnect.org',
  ...
  logLevel: (process.env.WALLETCONNECT_LOG_LEVEL || 'info') as
    | 'debug'
    | 'info'
    | 'warn'
    | 'error',
  ...
  deploymentApprovalRequired:
    process.env.WALLETCONNECT_DEPLOYMENT_APPROVAL_REQUIRED === 'true',
  requireSecurityAudit:
    process.env.WALLETCONNECT_REQUIRE_SECURITY_AUDIT === 'true',
  requiredNamespaces: {
    eip155: {
      methods: [
        'eth_sendTransaction',
...
export const createWalletConnectMainnetStagingSession = () => ({
  ...
  expiry:
    ...
    walletConnectMainnetStagingConfig.sessionConfig.expiry,
  namespaces: walletConnectMainnetStagingConfig.requiredNamespaces,
  securityContext: {
    approvalRequired:
      walletConnectMainnetStagingConfig.deploymentApprovalRequired,
    securityAuditCompleted:
      walletConnectMainnetStagingConfig.requireSecurityAudit,
...
export const validateMainnetStagingDeployment = async (): Promise<boolean> => {
  const checks = {
    ...
    encryptionEnabled:
      walletConnectMainnetStagingConfig.securityRequirements.encryptionRequired,
    mfaEnabled:
      walletConnectMainnetStagingConfig.securityRequirements.mfaRequired,
  };
