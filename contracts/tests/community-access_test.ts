import { Clarinet, Tx, Chain, Account, types } from 'https://deno.land/x/clarinet@v1.0.0/index.ts';
import { assertEquals } from 'https://deno.land/std@0.90.0/testing/asserts.ts';

Clarinet.test({
    name: "Can manage community issuer roles",
    async fn(chain: Chain, accounts: Map<string, Account>) {
        const deployer = accounts.get('deployer')!;
        const wallet1 = accounts.get('wallet_1')!;
        const communityId = 1;

        let block = chain.mineBlock([
            // Grant role
            Tx.contractCall('access-control', 'grant-community-issuer-role', [
                types.uint(communityId),
                types.principal(wallet1.address)
            ], deployer.address),
            // Check role
            Tx.contractCall('access-control', 'is-community-issuer', [
                types.uint(communityId),
                types.principal(wallet1.address)
            ], deployer.address)
        ]);

        assertEquals(block.receipts.length, 2);
        block.receipts[0].result.expectOk();
        assertEquals(block.receipts[1].result, types.bool(true));

        block = chain.mineBlock([
            // Revoke role
            Tx.contractCall('access-control', 'revoke-community-issuer-role', [
                types.uint(communityId),
                types.principal(wallet1.address)
            ], deployer.address),
            // Check role again
            Tx.contractCall('access-control', 'is-community-issuer', [
                types.uint(communityId),
                types.principal(wallet1.address)
            ], deployer.address)
        ]);

        assertEquals(block.receipts.length, 2);
        block.receipts[0].result.expectOk();
        assertEquals(block.receipts[1].result, types.bool(false));
    },
});

Clarinet.test({
    name: "Community issuer can mint badge for their community",
    async fn(chain: Chain, accounts: Map<string, Account>) {
        const deployer = accounts.get('deployer')!;
        const issuer = accounts.get('wallet_1')!;
        const recipient = accounts.get('wallet_2')!;
        const communityId = 1;
        const templateId = 1;

        let block = chain.mineBlock([
            // Grant issuer role
            Tx.contractCall('access-control', 'grant-community-issuer-role', [
                types.uint(communityId),
                types.principal(issuer.address)
            ], deployer.address),
            // Mint badge as community issuer
            Tx.contractCall('badge-issuer', 'mint-badge', [
                types.principal(recipient.address),
                types.uint(templateId),
                types.uint(communityId)
            ], issuer.address)
        ]);

        assertEquals(block.receipts.length, 2);
        block.receipts[0].result.expectOk();
        block.receipts[1].result.expectOk();
    },
});

Clarinet.test({
    name: "Unauthorized user cannot mint badge for community",
    async fn(chain: Chain, accounts: Map<string, Account>) {
        const unauthorized = accounts.get('wallet_3')!;
        const recipient = accounts.get('wallet_2')!;
        const communityId = 1;
        const templateId = 1;

        let block = chain.mineBlock([
            Tx.contractCall('badge-issuer', 'mint-badge', [
                types.principal(recipient.address),
                types.uint(templateId),
                types.uint(communityId)
            ], unauthorized.address)
        ]);

        assertEquals(block.receipts.length, 1);
        block.receipts[0].result.expectErr().expectUint(104); // ERR-UNAUTHORIZED
    },
});
