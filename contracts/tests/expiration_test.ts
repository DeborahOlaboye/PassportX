...
Clarinet.test({
    ...
    async fn(chain: Chain, accounts: Map<string, Account>) {
        ...
        let metadataBlock = chain.mineBlock([
            Tx.contractCall('badge-metadata', 'get-badge-metadata', [types.uint(1)], deployer.address),
            Tx.contractCall('badge-metadata', 'is-badge-expired', [types.uint(1)], deployer.address),
            Tx.contractCall('badge-metadata', 'is-badge-valid', [types.uint(1)], deployer.address)
        ]);

        let metadata = metadataBlock.receipts[0].result.expectSome().expectTuple();
        ...
        let expiredBlock = chain.mineBlock([
            Tx.contractCall('badge-metadata', 'is-badge-expired', [types.uint(1)], deployer.address),
            Tx.contractCall('badge-metadata', 'is-badge-valid', [types.uint(1)], deployer.address)
        ]);
...
Clarinet.test({
    ...
    async fn(chain: Chain, accounts: Map<string, Account>) {
        ...
        let renewBlock = chain.mineBlock([
            Tx.contractCall('badge-issuer', 'renew-badge', [
                types.uint(1),
                types.uint(communityId)
            ], deployer.address)
        ]);
