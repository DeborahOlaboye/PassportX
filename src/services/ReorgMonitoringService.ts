...
export class ReorgMonitoringService {
   ...
   */
  async recordReorgEvent(
    reorgEvent: ReorgEvent,
    database?: ReorgAwareDatabase
  ): Promise<void> {
  ...
  getMetricsHistory(hours: number = 24): ReorgMetrics[] {
    ...
    return this.metricsHistory.filter(
      (m) => m.lastReorgTimestamp >= cutoffTime
    );
  }
  ...
  private updateMetrics(reorgEvent: ReorgEvent): void {
    const rollbackDepth =
      reorgEvent.newCanonicalBlock - reorgEvent.rollbackToBlock;

    // Update counters
    this.metrics.totalReorgs++;
    this.metrics.totalAffectedTransactions +=
      reorgEvent.affectedTransactions.length;
    ...
    const totalDepth =
      this.metrics.averageRollbackDepth * (this.metrics.totalReorgs - 1) +
      rollbackDepth;
    this.metrics.averageRollbackDepth = totalDepth / this.metrics.totalReorgs;

    // Update depth distribution
    this.metrics.reorgByDepth[rollbackDepth] =
      (this.metrics.reorgByDepth[rollbackDepth] || 0) + 1;
    ...
    const recentReorgs = this.metricsHistory.filter(
      (m) => m.lastReorgTimestamp >= Date.now() - 24 * 60 * 60 * 1000
    ).length;
  ...
  private checkForAlerts(reorgEvent: ReorgEvent, dbStats?: any): ReorgAlert[] {
    ...
    if (reorgEvent.affectedTransactions.length > 100) {
      alerts.push({
        ...
        type: 'large_impact',
        severity:
          reorgEvent.affectedTransactions.length > 500 ? 'critical' : 'high',
    ...
    const recentReorgs = this.metricsHistory.filter(
      (m) => m.lastReorgTimestamp >= Date.now() - 60 * 60 * 1000
    ).length;
  ...
  private calculateTrends(recentMetrics: ReorgMetrics[]): {
    ...
    const firstHalfFreq =
      firstHalf.reduce((sum, m) => sum + m.reorgFrequency, 0) /
      firstHalf.length;
    const secondHalfFreq =
      secondHalf.reduce((sum, m) => sum + m.reorgFrequency, 0) /
      secondHalf.length;

    const firstHalfDepth =
      firstHalf.reduce((sum, m) => sum + m.averageRollbackDepth, 0) /
      firstHalf.length;
    const secondHalfDepth =
      secondHalf.reduce((sum, m) => sum + m.averageRollbackDepth, 0) /
      secondHalf.length;
    ...
    return {
      reorgFrequencyTrend:
        Math.abs(secondHalfFreq - firstHalfFreq) < freqThreshold
          ? 'stable'
          : secondHalfFreq > firstHalfFreq
          ? 'increasing'
          : 'decreasing',
      averageDepthTrend:
        Math.abs(secondHalfDepth - firstHalfDepth) < depthThreshold
          ? 'stable'
          : secondHalfDepth > firstHalfDepth
          ? 'increasing'
          : 'decreasing',
    };
  }
  ...
  private generateRecommendations(trends: any): string[] {
    ...
    if (trends.reorgFrequencyTrend === 'increasing') {
      recommendations.push(
        'Reorg frequency is increasing. Consider increasing block confirmation requirements.'
      );
    }

    if (trends.averageDepthTrend === 'increasing') {
      recommendations.push(
        'Average reorg depth is increasing. Monitor network stability.'
      );
    }

    if (this.metrics.maxRollbackDepth > 100) {
      recommendations.push(
        'Very deep reorgs detected. Consider implementing additional safety measures.'
      );
    }

    if (this.alerts.filter((a) => a.severity === 'critical').length > 0) {
      recommendations.push(
        'Critical reorg alerts detected. Immediate investigation recommended.'
      );
    }
  ...
  private getDefaultLogger() {
    return {
      debug: (msg: string, ...args: any[]) =>
        console.debug(`[ReorgMonitoringService] ${msg}`, ...args),
      info: (msg: string, ...args: any[]) =>
        console.info(`[ReorgMonitoringService] ${msg}`, ...args),
      warn: (msg: string, ...args: any[]) =>
        console.warn(`[ReorgMonitoringService] ${msg}`, ...args),
      error: (msg: string, ...args: any[]) =>
        console.error(`[ReorgMonitoringService] ${msg}`, ...args),
    };
  }
}
