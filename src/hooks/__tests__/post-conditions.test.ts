...
describe('Post-Condition Validation Tests', () => {
  describe('STX Post-Condition Validation', () => {
    it('should validate STX send-greater-than condition', () => {
      ...
      expect(PostConditionValidator.validateSTX(condition, 1001).valid).toBe(true)

      // Invalid case: amount <= required
      expect(PostConditionValidator.validateSTX(condition, 1000).valid).toBe(false)
      expect(PostConditionValidator.validateSTX(condition, 999).valid).toBe(false)
    })
    ...
    it('should validate STX send-greater-than-or-equal condition', () => {
      ...
      // Valid cases
      expect(PostConditionValidator.validateSTX(condition, 1001).valid).toBe(true)
      expect(PostConditionValidator.validateSTX(condition, 1000).valid).toBe(true)
    ...
    it('should validate STX send-less-than condition', () => {
      ...
      // Valid case
      expect(PostConditionValidator.validateSTX(condition, 999).valid).toBe(true)

      // Invalid cases
      expect(PostConditionValidator.validateSTX(condition, 1000).valid).toBe(false)
      expect(PostConditionValidator.validateSTX(condition, 1001).valid).toBe(false)
    })
  ...
  describe('Fungible Token Post-Condition Validation', () => {
    it('should validate FT post-conditions', () => {
      ...
      // Valid case
      expect(PostConditionValidator.validateFT(condition, 5001).valid).toBe(true)

      // Invalid case
      expect(PostConditionValidator.validateFT(condition, 4999).valid).toBe(false)
    })
  ...
  describe('Badge Issuance Post-Conditions', () => {
    it('should validate badge issuance fee post-condition', () => {
      ...
      expect(PostConditionValidator.validateSTX(feeCondition, 100).valid).toBe(true)

      // Valid: paying more than the fee
      expect(PostConditionValidator.validateSTX(feeCondition, 200).valid).toBe(true)

      // Invalid: paying less than the fee
      expect(PostConditionValidator.validateSTX(feeCondition, 99).valid).toBe(false)
    })
  ...
  describe('Community Creation Post-Conditions', () => {
    it('should validate community creation payment post-condition', () => {
      ...
      expect(PostConditionValidator.validateSTX(paymentCondition, 1000).valid).toBe(true)

      // Invalid: paying different amount
      expect(PostConditionValidator.validateSTX(paymentCondition, 999).valid).toBe(false)
      expect(PostConditionValidator.validateSTX(paymentCondition, 1001).valid).toBe(false)
    })
  ...
  describe('Post-Condition Edge Cases', () => {
    it('should handle zero amounts', () => {
      ...
      expect(PostConditionValidator.validateSTX(condition, 1).valid).toBe(false)
    })
    ...
    it('should handle very large amounts', () => {
      ...
      expect(PostConditionValidator.validateSTX(condition, 1000000000).valid).toBe(true)
      expect(PostConditionValidator.validateSTX(condition, 999999999).valid).toBe(false)
    })
  ...
  describe('Post-Condition Validation Errors', () => {
    ...
    it('should handle missing post-condition fields', () => {
      ...
      const result = PostConditionValidator.validateSTX(incompleteCondition, 1000)
      expect(result.valid).toBe(false)
    })
