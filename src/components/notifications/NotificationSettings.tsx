...
export default function NotificationSettings() {
  const { preferences, loading, updatePreference } = useNotificationPreferences('user-id');
      ...
      {Object.values(NotificationType).map((type) => {
        ...
        return (
          <div key={type} className="border rounded-lg p-4">
            <h3 className="font-semibold mb-3 capitalize">{type.replace(/_/g, ' ')}</h3>
            <div className="space-y-2">
              <label className="flex items-center gap-3 cursor-pointer">
                <input
                  type="checkbox"
                  checked={pref?.channels.includes(NotificationChannel.IN_APP)}
                  onChange={() => handleToggle(type, NotificationChannel.IN_APP)}
                  className="w-4 h-4"
                />
              ...
              <label className="flex items-center gap-3 cursor-pointer">
                <input
                  type="checkbox"
                  checked={pref?.channels.includes(NotificationChannel.WEBSOCKET)}
                  onChange={() => handleToggle(type, NotificationChannel.WEBSOCKET)}
                  className="w-4 h-4"
                />
