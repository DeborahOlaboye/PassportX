...
export class ChainhookNotificationIntegration {
  ...
  static initialize(): void {
    ...
    console.log(
      'ChainhookNotificationIntegration initialized with default handlers'
    );
  }
  ...
  private static registerDefaultHandlers(): void {
    ...
    NotificationService.registerHandler(
      badgeMintHandler.getEventType(),
      badgeMintHandler
    );
    NotificationService.registerHandler(
      badgeVerificationHandler.getEventType(),
      badgeVerificationHandler
    );
    NotificationService.registerHandler(
      communityUpdateHandler.getEventType(),
  ...
  private static async deliverNotifications(
    ...
    for (const notification of notifications) {
      try {
        ...
        const shouldSend = userPreferences
          ? !(
              (notification.type === 'badge_received' &&
                !userPreferences.badges.mint) ||
              (notification.type === 'badge_verified' &&
                !userPreferences.badges.verify) ||
              (notification.type === 'community_update' &&
        ...
        if (!shouldSend) {
          console.log(
            `Skipping notification for user ${notification.userId} due to preferences`
          );
          continue;
        }
  ...
  private static async emitNotificationEvents(
    notifications: NotificationPayload[]
  ): Promise<void> {
    for (const notification of notifications) {
      try {
