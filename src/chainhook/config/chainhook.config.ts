import {
  IChainhookConfig,
  IChainhookServerOptions,
  IChainhookNodeOptions,
} from '../types/chainhook.types';
...
const chainhookConfig: IChainhookConfig = {
  ...
  network: (process.env.STACKS_NETWORK || 'devnet') as
    | 'devnet'
    | 'testnet'
    | 'mainnet',
};
...
export const getServerOptions = (): IChainhookServerOptions => ({
  ...
  host: '0.0.0.0',
  logLevel: (process.env.NODE_ENV === 'production' ? 'info' : 'debug') as
    | 'debug'
    | 'info'
    | 'warn'
    | 'error',
