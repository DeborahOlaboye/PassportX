...
export function validateCustomUrl(url: string): ValidationResult {
  ...
  if (url.length < 3) {
    return {
      isValid: false,
      error: 'Custom URL must be at least 3 characters long',
    };
  }
  ...
  if (url.length > 30) {
    return {
      isValid: false,
      error: 'Custom URL must be at most 30 characters long',
    };
  }
  ...
  if (url.startsWith('-') || url.endsWith('-')) {
    return {
      isValid: false,
      error: 'Custom URL cannot start or end with a hyphen',
    };
  }
  ...
  if (url.includes('--')) {
    return {
      isValid: false,
      error: 'Custom URL cannot contain consecutive hyphens',
    };
  }
...
export function validateSocialUsername(
  ...
  if (cleanUsername.length > 50) {
    return {
      isValid: false,
      error: `${platform} username must be at most 50 characters`,
    };
  }
...
export function validateHexColor(color: string): ValidationResult {
  ...
  if (!hexRegex.test(color)) {
    return {
      isValid: false,
      error: 'Please enter a valid hex color code (e.g., #3B82F6)',
    };
  }
...
export function validateProfileData(data: ProfileData): ValidationResult {
  ...
  if (data.socialLinks) {
    if (data.socialLinks.twitter) {
      const result = validateSocialUsername(
