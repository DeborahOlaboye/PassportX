...
function transformAnalyticsResponse(data: any): AnalyticsData {
  ...
  const badgesChange =
    totalBadgesBeforeMonth > 0
      ? Math.round(
          ((totalBadgesLastMonth - totalBadgesBeforeMonth) /
  ...
  const totalInteractions =
    timeSeries.reduce((sum: number, item: any) => sum + item.activeUsers, 0) ||
    0;

  const avgInteractionPerUser =
    users.totalUsers > 0 ? Math.round(totalInteractions / users.totalUsers) : 0;

  const peakTime = getPeakTime(dailyActiveUsers);

  const weeklyGrowth = calculateGrowthRate(dailyNewUsers.slice(-7));
  const monthlyGrowth = calculateGrowthRate(dailyNewUsers.slice(-30));
