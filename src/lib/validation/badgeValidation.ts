...
const MAX_RECIPIENT_NAME_LENGTH =
  VALIDATION_CONSTANTS.MAX_RECIPIENT_NAME_LENGTH;
const MIN_RECIPIENT_NAME_LENGTH =
  VALIDATION_CONSTANTS.MIN_RECIPIENT_NAME_LENGTH;
const MIN_STACKS_ADDRESS_LENGTH =
  VALIDATION_CONSTANTS.MIN_STACKS_ADDRESS_LENGTH;
...
const MAX_BADGE_DESCRIPTION_LENGTH =
  VALIDATION_CONSTANTS.MAX_BADGE_DESCRIPTION_LENGTH;
const MIN_BADGE_DESCRIPTION_LENGTH =
...
export const validateStacksAddress = (
  address: string
...
export const validateTemplateSelection = (
  templateId: number
): ValidationError | null => {
...
export const validateCommunitySelection = (
  communityId: number
): ValidationError | null => {
...
export const validateBadgeCategory = (
  ...
  const categoriesToCheck =
    validCategories || Array.from(VALID_BADGE_CATEGORIES);
...
export const validateBadgeMetadata = (data: {
  ...
  const categoryError = validateBadgeCategory(
    data.category,
...
export const validateBulkBadgeIssuance = (
  recipients: Array<{
    name: string;
    address: string;
    email?: string;
  ...
  if (recipients.length > 1000) {
    errors.push({
      field: 'recipients',
      message: 'Cannot issue to more than 1000 recipients at once',
    });
  }
...
export const isLevelInValidRange = (level: number): boolean => {
  return (
    ...
    level >= MIN_BADGE_LEVEL &&
    level <= MAX_BADGE_LEVEL
  );
};
