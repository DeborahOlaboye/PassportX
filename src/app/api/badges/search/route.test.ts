...
describe('Badges Search API Route', () => {
  describe('GET', () => {
    it('should search badges with query params successfully', async () => {
      ...
      const request = new NextRequest(new URL('http://localhost:3000/api/badges/search?q=test'))
    ...
    it('should handle backend errors on GET', async () => {
      ...
      const request = new NextRequest(new URL('http://localhost:3000/api/badges/search'))
      const response = await GET(request)
