...
export class TestnetConnectionManager {
  ...
  async connect(): Promise<boolean> {
    try {
      ...
      const response = await fetch(`${testnetConfig.network.coreApiUrl}/v2/info`)
   ...
   */
  async verifyContract(address: string, contractName: string): Promise<boolean> {
    if (!this.isConnected) {
    ...
    try {
      ...
      console.error(`Failed to verify contract ${address}.${contractName}:`, error)
      return false
    }
  }
...
export class ContractDeploymentVerifier {
   ...
   */
  async verifyAllContracts(): Promise<{ contract: string; deployed: boolean }[]> {
   ...
   */
  async verifyContractFunctions(address: string, contractName: string): Promise<boolean> {
    try {
      const url = `${testnetConfig.network.coreApiUrl}/v2/contracts/interface/${address}/${contractName}`
      const response = await fetch(url)
   ...
   */
  async verifyAccountBalances(): Promise<{ account: string; balance: number; sufficient: boolean }[]> {
    ...
    for (const [key, account] of Object.entries(testnetConfig.accounts)) {
      try {
        const balance = await this.connectionManager.getBalance(account.stxAddress)
        ...
        if (!sufficient) {
          console.warn(`⚠️  Account ${key} has insufficient balance: ${balance / 1000000} STX`)
        } else {
