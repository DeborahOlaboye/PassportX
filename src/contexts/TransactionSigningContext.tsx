...
} from '@/types/transaction-signing';
import { useWalletConnect } from '@/contexts/WalletConnectContext';

const TransactionSigningContext = createContext<
  TransactionSigningState | undefined
>(undefined);

export function TransactionSigningProvider({
  children,
}: {
  ...
  const [signedTransactions, setSignedTransactions] = useState<
    SignedTransaction[]
  >([]);
  const { signTransaction: walletSign, broadcastTransaction: walletBroadcast } =
    useWalletConnect();

  const signTransaction = useCallback(
    async (request: TransactionRequest): Promise<SignedTransaction> => {
      setIsSigning(true);
  ...
  const estimateGas = useCallback(
    async (request: TransactionRequest): Promise<GasEstimate> => {
...
export function useTransactionSigning() {
  ...
  if (context === undefined) {
    throw new Error(
      'useTransactionSigning must be used within a TransactionSigningProvider'
    );
  }
  return context;
}
