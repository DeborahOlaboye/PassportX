...
function validateBadgeMintEvent(event: any): {
  valid: boolean;
  errors: string[];
} {
  const errors: string[] = [];
  ...
  async (req: Request, res: Response) => {
    try {
      if (!badgeMintService || !notificationService || !cacheService) {
        logger.error('Badge mint services not initialized');
      ...
      try {
        ...
        logger.error(
          'Error sending badge mint notifications:',
          notificationError
        );
        res.status(201).json({
          success: true,
      ...
      logger.error('Error processing badge mint webhook:', error);
      res.status(500).json({
        success: false,
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      if (!badgeId || !contractAddress || !recipientAddress || !badgeName) {
        return res.status(400).json({
          error:
            'Missing required fields: badgeId, contractAddress, recipientAddress, badgeName',
        });
      }
      ...
      logger.error('Error syncing badge from blockchain:', error);
      res.status(500).json({
...
router.get('/status/:badgeId', async (req: Request, res: Response) => {
  try {
    ...
    logger.error('Error checking badge mint status:', error);
    res.status(500).json({
...
router.post(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      logger.error('Error generating test badge mint notification:', error);
      res.status(500).json({
...
router.get('/cache/stats', authenticateToken, (req: Request, res: Response) => {
  try {
    ...
    logger.error('Error getting badge cache stats:', error);
    res.status(500).json({
...
router.post(
  ...
  (req: Request, res: Response) => {
    try {
      ...
      logger.error('Error clearing badge cache:', error);
      res.status(500).json({
