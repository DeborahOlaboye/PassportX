  ...
  async (req: Request, res: Response) => {
    try {
      if (!metricsTracker) {
        return res
          .status(503)
          .json({ error: 'Metrics tracker not initialized' });
      }
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      if (status) {
        ...
        events = await eventLogger.getEventsByStatus(
          'completed',
          limit,
          offset
        );
  ...
  async (req: Request, res: Response) => {
    try {
      if (!metricsTracker || !eventLogger || !healthMonitor || !alertService) {
        return res
          .status(503)
          .json({ error: 'Monitoring services not fully initialized' });
      }
      ...
      const avgProcessingTime = await metricsTracker.getAverageProcessingTime(
        1
      );
