...
export class UserAcquisitionAnalytics {
  ...
  private cache: Map<
    string,
    { data: UserAcquisitionMetrics; timestamp: number }
  > = new Map();
  ...
  async getAcquisitionMetrics(): Promise<UserAcquisitionMetrics> {
    ...
    try {
      ...
      const avgBadgesPerUser =
        totalUsers > 0 ? Math.round(totalBadges / totalUsers) : 0;
  ...
  private async getDailyNewUsers(
    days: number
  ): Promise<UserAcquisitionMetrics['dailyNewUsers']> {
  ...
  private async getWeeklyNewUsers(
    weeks: number
  ): Promise<UserAcquisitionMetrics['weeklyNewUsers']> {
    ...
    for (let i = weeks - 1; i >= 0; i--) {
      ...
      result.push({
        week: `${weekStart.toISOString().split('T')[0]} to ${
  ...
  private async getMonthlyNewUsers(
    months: number
  ): Promise<UserAcquisitionMetrics['monthlyNewUsers']> {
  ...
  private async getActiveUsersInRange(
    startDate: Date,
    endDate: Date
  ): Promise<number> {
  ...
  private async getDailyActiveUsers(
    days: number
  ): Promise<UserAcquisitionMetrics['dailyActiveUsers']> {
  ...
  private async calculateRetentionRate(): Promise<number> {
    ...
    const originalOwners = new Set(
      originalUsers.map((u) => u.owner || u._id?.toString())
    );
  ...
  private async getRetentionCohorts(): Promise<
    UserAcquisitionMetrics['userRetentionCohorts']
  > {
    ...
    for (let monthOffset = 0; monthOffset < 3; monthOffset++) {
      ...
      const owners = new Set(
        cohortUsers.map((u) => u.owner || u._id?.toString())
      );
      const retainedUsers = await Badge.distinct('owner', {
  ...
  private async getUserEngagementMetrics(): Promise<
    UserAcquisitionMetrics['userEngagement']
  > {
