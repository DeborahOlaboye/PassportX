...
import UserActivityFeed, {
  IUserActivityFeed,
  ActivityEventType,
} from '../models/UserActivityFeed';
import {
...
export class UserActivityService extends EventEmitter {
  ...
  async recordBadgeReceivedEvent(data: {
    ...
    try {
      ...
      this.logger.info('Recorded badge received event', {
        userId: data.userId,
        badgeId: data.badgeId,
      });
  ...
  async recordBadgeRevokedEvent(data: {
    ...
    try {
      ...
      const activity = new UserActivityFeed({
        ...
        title: `Badge ${
          data.revocationType === 'hard' ? 'Revoked' : 'Suspended'
        }: ${data.badgeName}`,
      ...
      const savedActivity = await activity.save();
      this.logger.info('Recorded badge revoked event', {
        userId: data.userId,
        badgeId: data.badgeId,
      });
  ...
  async recordCommunityJoinedEvent(data: {
    ...
    try {
      ...
      const savedActivity = await activity.save();
      this.logger.info('Recorded community joined event', {
        userId: data.userId,
        communityId: data.communityId,
      });
  ...
  async recordCommunityCreatedEvent(data: {
    ...
    try {
      ...
      const savedActivity = await activity.save();
      this.logger.info('Recorded community created event', {
        userId: data.userId,
        communityId: data.communityId,
      });
  ...
  async recordBadgeMetadataUpdatedEvent(data: {
    ...
    try {
      ...
      const savedActivity = await activity.save();
      this.logger.info('Recorded badge metadata updated event', {
        userId: data.userId,
        badgeId: data.badgeId,
      });
  ...
  async recordPassportCreatedEvent(data: {
    ...
    try {
      ...
      const savedActivity = await activity.save();
      this.logger.info('Recorded passport created event', {
        userId: data.userId,
      });
  ...
  async getUserActivityFeed(
    ...
    try {
      ...
      const limit =
        Math.min(options.limit, this.MAX_PAGE_SIZE) || this.DEFAULT_PAGE_SIZE;
      ...
      this.logger.debug('Retrieved user activity feed', {
        userId,
        page,
        limit,
        total,
  ...
  async markActivityAsRead(
    activityId: string
  ): Promise<IUserActivityFeed | null> {
    try {
      ...
      if (activity) {
        ...
        this.emit('activity:marked-read', {
          activityId,
  ...
  async markAllActivitiesAsRead(userId: string): Promise<number> {
    try {
      ...
      this.logger.info('Marked all activities as read', {
        userId,
        count: result.modifiedCount,
      });
      this.emit('activities:marked-read-all', {
        userId,
        count: result.modifiedCount,
      });
      broadcastActivityUpdate(userId, 'marked-read-all', {
        count: result.modifiedCount,
      });
  ...
  async getUnreadCount(userId: string): Promise<number> {
    try {
      const count = await UserActivityFeed.countDocuments({
        userId,
        isRead: false,
      });
      return count;
  ...
  /**
   * Delete an activity only if it belongs to the given userId.
   * Returns true on success, false if not found, null if the activity
   * exists but is owned by a different user (forbidden).
   */
  async deleteActivityForUser(
    activityId: string,
    userId: string
  ): Promise<boolean | null> {
    try {
      const activity = await UserActivityFeed.findById(activityId).select(
        'userId'
      );
      if (!activity) {
        return false;
      ...
      this.logger.error('Error deleting activity for user:', error);
      throw error;
    }
  }
  ...
  async clearUserActivities(userId: string): Promise<number> {
    try {
      ...
      this.logger.info('Cleared user activities', {
        userId,
        count: result.deletedCount,
      });
      this.emit('activities:cleared', { userId, count: result.deletedCount });
      broadcastActivityUpdate(userId, 'cleared', {
        count: result.deletedCount,
      });
