...
export class ChainhookEventDeduplicator {
  private eventHashes: Map<string, { timestamp: number; count: number }> =
    new Map();
  ...
  private getDefaultLogger() {
    return {
      debug: (msg: string, ...args: any[]) =>
        console.debug(`[Deduplicator] ${msg}`, ...args),
      info: (msg: string, ...args: any[]) =>
        console.info(`[Deduplicator] ${msg}`, ...args),
      warn: (msg: string, ...args: any[]) =>
        console.warn(`[Deduplicator] ${msg}`, ...args),
      error: (msg: string, ...args: any[]) =>
        console.error(`[Deduplicator] ${msg}`, ...args),
    };
  }
  ...
  isDuplicate(event: any): boolean {
    ...
    if (existing && now - existing.timestamp < this.config.windowSizeMs) {
      ...
      this.logger.debug(
        `Duplicate event detected (count: ${existing.count}): ${hash}`
      );
      return true;
    }
  ...
  private updateDeduplicationRate(): void {
    if (this.metrics.totalEvents > 0) {
      this.metrics.deduplicationRate =
        (this.metrics.duplicatesDetected / this.metrics.totalEvents) * 100;
    }
  }
  ...
  private startCleanup(): void {
    this.cleanupInterval = setInterval(() => {
      ...
      if (cleanedCount > 0) {
        this.logger.debug(
          `Cleanup removed ${cleanedCount} expired event hashes`
        );
      }
