...
export class BadgeDistributionAnalytics {
  private logger: any;
  private cache: Map<string, { data: BadgeDistribution; timestamp: number }> =
    new Map();
  ...
  private async getBadgesByCategory(): Promise<
    BadgeDistribution['byCategory']
  > {
  ...
  private async getTopBadges(
    limit: number
  ): Promise<BadgeDistribution['topBadges']> {
    const result = await Badge.aggregate([
      {
        $group: {
          _id: '$templateId',
  ...
  private async getTrendingBadges(
    limit: number
  ): Promise<BadgeDistribution['trendingBadges']> {
    const now = new Date();
    ...
    const trending = Array.from(merged.entries())
      .map(([templateId, counts]) => ({
        ...
        growth:
          counts.last30 > 0
            ? ((counts.last7 - counts.last30 / 4) / (counts.last30 / 4)) * 100
            : 0,
        last7: counts.last7,
  ...
  private async getBadgesByIssuer(): Promise<
    BadgeDistribution['badgesByIssuer']
  > {
    const result = await Badge.aggregate([
      {
        $group: {
