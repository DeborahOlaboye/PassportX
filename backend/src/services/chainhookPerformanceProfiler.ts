...
export class ChainhookPerformanceProfiler {
  ...
  private getDefaultLogger() {
    return {
      debug: (msg: string, ...args: any[]) =>
        console.debug(`[Profiler] ${msg}`, ...args),
      info: (msg: string, ...args: any[]) =>
        console.info(`[Profiler] ${msg}`, ...args),
      warn: (msg: string, ...args: any[]) =>
        console.warn(`[Profiler] ${msg}`, ...args),
      error: (msg: string, ...args: any[]) =>
        console.error(`[Profiler] ${msg}`, ...args),
    };
  }
  ...
  getSnapshot(): ProfilerSnapshot {
    ...
    const eventThroughput =
      timeSinceCheck > 0
        ? (totalEventCount - this.lastEventCount) / timeSinceCheck
        : 0;
  ...
  printReport(limit: number = 10): void {
    ...
    for (const [name, metric] of sorted.slice(0, limit)) {
      ...
      this.logger.info(
        `  Count: ${metric.count}, Avg: ${metric.avg.toFixed(
          2
        )}ms, Min: ${metric.min.toFixed(2)}ms, Max: ${metric.max.toFixed(2)}ms`
      );
      this.logger.info(
        `  P50: ${metric.p50.toFixed(2)}ms, P95: ${metric.p95.toFixed(
          2
        )}ms, P99: ${metric.p99.toFixed(2)}ms`
      );
    }

    const snapshot = this.getSnapshot();
    this.logger.info(
      `Event Throughput: ${snapshot.systemMetrics.eventThroughput.toFixed(
        2
      )} events/sec`
    );
    this.logger.info(
      `Memory Usage: ${(
        snapshot.systemMetrics.memoryUsage.heapUsed /
        1024 /
        1024
      ).toFixed(2)}MB`
    );
