...
export class ErrorMonitoringService {
  ...
  recordError(
    ...
    this.logger.error(
      `Error recorded: [${service}] ${type} - ${message}`,
      metadata
    );
  ...
  private async checkThreshold(threshold: AlertThreshold): Promise<void> {
    ...
    switch (threshold.metric) {
      case 'error_rate':
        ...
        message = `Error rate is ${currentValue.toFixed(
          2
        )} errors/minute (threshold: ${threshold.threshold})`;
        break;
      ...
      case 'retry_queue_size':
        currentValue = await RetryQueue.countDocuments({
  ...
  private async sendSlackNotification(
    ...
    const payload = {
      text: `${
        severityEmoji[notification.severity as keyof typeof severityEmoji] ||
        '⚠️'
      } ${notification.title}`,
      blocks: [
        {
  ...
  async getStatistics(): Promise<{
    ...
    const retryQueueSize = await RetryQueue.countDocuments({
      status: { $in: ['pending', 'retrying'] },
    });
  ...
  async getHealthStatus(): Promise<{
    ...
    if (metrics.errorRate > 10) {
      issues.push(
        `High error rate: ${metrics.errorRate.toFixed(2)} errors/minute`
      );
      status = 'degraded';
    }
