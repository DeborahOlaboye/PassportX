  ...
  private getDefaultLogger() {
    return {
      debug: (msg: string, ...args: any[]) =>
        console.debug(`[HealthMonitor] ${msg}`, ...args),
      info: (msg: string, ...args: any[]) =>
        console.info(`[HealthMonitor] ${msg}`, ...args),
      warn: (msg: string, ...args: any[]) =>
        console.warn(`[HealthMonitor] ${msg}`, ...args),
      error: (msg: string, ...args: any[]) =>
        console.error(`[HealthMonitor] ${msg}`, ...args),
    };
  }
  ...
  private async recordHealthCheck(
    ...
    try {
      ...
      if (existingHealth) {
        ...
        if (isConnected) {
          updates.successfulAttempts =
            (existingHealth.successfulAttempts || 0) + 1;
          updates.failedAttempts = Math.max(
            0,
            (existingHealth.failedAttempts || 1) - 1
          );
        } else {
