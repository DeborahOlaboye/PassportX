import ChainhookEventObserverService, {
  ChainhookEventObserverService as ObserverService,
} from './chainhookEventObserver';
...
export class ChainhookManager {
  ...
  async initialize(): Promise<void> {
    try {
      ...
      this.logger.error(
        'Failed to initialize ChainhookManager',
        error as Error
      );
      throw error;
    }
    ...
    try {
      ...
      if (eventType) {
        this.subscriptionManager
          .dispatchEventToAll(event, eventType)
          .catch((error) => {
            this.logger.error(
              'Error dispatching event to subscriptions',
              error as Error
            );
