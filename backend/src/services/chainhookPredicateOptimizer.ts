...
export class ChainhookPredicateOptimizer {
  ...
  private getDefaultLogger() {
    return {
      debug: (msg: string, ...args: any[]) =>
        console.debug(`[PredicateOptimizer] ${msg}`, ...args),
      info: (msg: string, ...args: any[]) =>
        console.info(`[PredicateOptimizer] ${msg}`, ...args),
      warn: (msg: string, ...args: any[]) =>
        console.warn(`[PredicateOptimizer] ${msg}`, ...args),
      error: (msg: string, ...args: any[]) =>
        console.error(`[PredicateOptimizer] ${msg}`, ...args),
    };
  }
  ...
  private createFilterFunction(
    ...
    return (event: any) => {
      if (
        filter.minBlockHeight !== undefined &&
        event.block_identifier?.index < filter.minBlockHeight
      ) {
        return false;
      }

      if (
        filter.maxBlockHeight !== undefined &&
        event.block_identifier?.index > filter.maxBlockHeight
      ) {
        return false;
      }
        ...
        for (const tx of event.transactions) {
          ...
          for (const op of tx.operations) {
            if (
              ...
              op.contract_call?.contract !== filter.contractAddress
            ) {
              continue;
            }
            ...
            if (filter.topic) {
              if (
                !op.events?.some((e: any) => e.topic?.includes(filter.topic))
              ) {
                continue;
              }
            }
  ...
  filterEvent(predicateId: string, event: any): boolean {
    ...
    try {
      ...
      if (!filter) {
        this.logger.warn(
          `No compiled filter found for predicate: ${predicateId}`
        );
        return true;
      }
      ...
      this.logger.error(
        `Error applying filter for predicate ${predicateId}:`,
        error
      );
      return true;
    }
  ...
  filterEventBatch(predicateId: string, events: any[]): any[] {
    ...
    this.logger.debug(
      `Batch filtering: ${events.length} → ${filtered.length} events`
    );
    return filtered;
  }
  ...
  private recordFilterTiming(time: number): void {
    ...
    if (this.metrics.eventsReceived > 0) {
      this.metrics.filteringRate =
        (this.metrics.eventsFiltered / this.metrics.eventsReceived) * 100;
    }
  }
