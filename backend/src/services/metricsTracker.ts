...
export class MetricsTracker {
  ...
  private getDefaultLogger() {
    return {
      debug: (msg: string, ...args: any[]) =>
        console.debug(`[MetricsTracker] ${msg}`, ...args),
      info: (msg: string, ...args: any[]) =>
        console.info(`[MetricsTracker] ${msg}`, ...args),
      warn: (msg: string, ...args: any[]) =>
        console.warn(`[MetricsTracker] ${msg}`, ...args),
      error: (msg: string, ...args: any[]) =>
        console.error(`[MetricsTracker] ${msg}`, ...args),
    };
  }
  ...
  trackEventProcessed(processingTime: number): void {
    ...
    if (this.currentMetrics.processingTimes.length > 1000) {
      this.currentMetrics.processingTimes =
        this.currentMetrics.processingTimes.slice(-1000);
    }
  }
