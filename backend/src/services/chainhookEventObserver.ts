import {
  ChainhookEventObserver as HiroChainhookEventObserver,
  ServerOptions,
...
export class ChainhookEventObserverService extends EventEmitter {
  ...
  constructor(config: ChainhookObserverConfig, logger?: any) {
    ...
    this.batcher = new ChainhookEventBatcher(
  ...
  async initialize(): Promise<void> {
    try {
      ...
      this.observer = new HiroChainhookEventObserver(
        serverOptions,
        nodeOptions
      );
  ...
  private async startLocalServer(): Promise<void> {
    return new Promise((resolve, reject) => {
      ...
      this.server.listen(this.config.serverPort, this.config.serverHost, () => {
        this.logger.info(
          `Chainhook event server listening on ${this.config.serverHost}:${this.config.serverPort}`
        );
        resolve();
      });
  ...
  private validateEvent(event: any): void {
    ...
    if (typeof event.block_identifier.index !== 'number') {
      throw new Error(
        'Invalid event structure: block_identifier.index must be a number'
      );
    }
  }
  ...
  private async handleConnectionError(error: any): Promise<void> {
    ...
    if (this.reconnectAttempts < this.maxReconnectAttempts) {
      ...
      this.logger.error(
        'Max reconnection attempts reached. Service will not auto-reconnect.'
      );
  ...
  unregisterPredicate(predicateName: string): void {
    try {
      ...
      this.logger.error(
        `Error unregistering predicate ${predicateName}:`,
        error
      );
      throw error;
    }
