  ...
  validateBlockEvent(event: any): ValidationResult {
    ...
    if (!event.block_identifier) {
      ...
      if (
        ...
        typeof event.block_identifier.index !== 'number'
      ) {
        errors.push('Invalid or missing block_identifier.index');
      }
      if (
        !event.block_identifier.hash ||
        typeof event.block_identifier.hash !== 'string'
      ) {
    ...
    if (!event.parent_block_identifier) {
      ...
      if (
        !event.parent_block_identifier.index ||
        typeof event.parent_block_identifier.index !== 'number'
      ) {
        errors.push('Invalid or missing parent_block_identifier.index');
      }
      if (
        !event.parent_block_identifier.hash ||
        typeof event.parent_block_identifier.hash !== 'string'
      ) {
  ...
  validateCompleteEvent(event: any): ValidationResult {
    ...
    if (Array.isArray(event.transactions)) {
      for (let i = 0; i < event.transactions.length; i++) {
        ...
        allErrors.push(
          ...txValidation.errors.map((e) => `Transaction ${i}: ${e}`)
        );
        allWarnings.push(
          ...txValidation.warnings.map((w) => `Transaction ${i}: ${w}`)
        );

        // Validate operations
        if (Array.isArray(event.transactions[i].operations)) {
          for (let j = 0; j < event.transactions[i].operations.length; j++) {
            const opValidation = this.validateOperation(
              event.transactions[i].operations[j]
            );
            allErrors.push(
              ...opValidation.errors.map(
                (e) => `Transaction ${i}, Operation ${j}: ${e}`
              )
            );
            allWarnings.push(
              ...opValidation.warnings.map(
                (w) => `Transaction ${i}, Operation ${j}: ${w}`
              )
            );
          }
        }
