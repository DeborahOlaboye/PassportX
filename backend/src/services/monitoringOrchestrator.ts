...
export class MonitoringOrchestrator {
  ...
  private getDefaultLogger() {
    return {
      debug: (msg: string, ...args: any[]) =>
        console.debug(`[MonitoringOrchestrator] ${msg}`, ...args),
      info: (msg: string, ...args: any[]) =>
        console.info(`[MonitoringOrchestrator] ${msg}`, ...args),
      warn: (msg: string, ...args: any[]) =>
        console.warn(`[MonitoringOrchestrator] ${msg}`, ...args),
      error: (msg: string, ...args: any[]) =>
        console.error(`[MonitoringOrchestrator] ${msg}`, ...args),
    };
  }
      ...
      if (!result.isConnected) {
        ...
        if (
          ...
          health.failedAttempts >=
            (this.config.alertThresholds?.maxConsecutiveFailures || 5)
        ) {
  ...
  private async startMetricsCollection(): Promise<void> {
    ...
    this.metricsInterval = setInterval(async () => {
      ...
      const connectionStatus = health?.isConnected
        ? 'connected'
        : 'disconnected';
  ...
  async shutdown(): Promise<void> {
    ...
    if (this.config.logRetentionDays) {
      const cleared = await this.eventLogger.clearOldLogs(
