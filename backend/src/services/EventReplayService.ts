...
export class EventReplayService {
  ...
  async getHistoricalEvents(
    ...
    try {
      ...
      const events = await ProcessedEvent.find(query)
        .sort({ processedAt: -1 })
        .limit(limit)
        .skip(offset)
        .lean();

      this.logger.info(`Retrieved ${events.length} historical events`, {
        filters,
      });
  ...
  async getEventStatistics(filters: EventReplayFilters = {}): Promise<{
    ...
    try {
      ...
      const [totalEvents, eventTypeStats, dateStats, blockStats] =
        await Promise.all([
  ...
  async replayEvents(
    ...
    try {
      const events = await this.getHistoricalEvents({
        ...filters,
        limit: 1000,
      }); // Limit replay to 1000 events at once
      ...
      for (const event of events) {
        try {
          ...
          if (
            ...
            Date.now() - event.lastReplayedAt.getTime() < 3600000
          ) {
            this.logger.warn(
              `Skipping replay of event ${event.id} - recently replayed`
            );
            continue;
          }

          // Re-process the event
          const processedEvents = await eventProcessor.processEvent(
            event.originalEvent
          );
          ...
          eventsReplayed++;
          this.logger.info(`Replayed event ${event.id}`, {
            eventType: event.eventType,
          });
        } catch (error) {
          const errorMsg = `Failed to replay event ${event.id}: ${
