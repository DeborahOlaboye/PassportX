...
export class ChainhookEventBatcher {
  ...
  constructor(config: Partial<BatchConfig> = {}, logger?: any) {
    ...
    this.logger.info(
      `EventBatcher initialized with batch size: ${this.config.batchSize}, timeout: ${this.config.batchTimeoutMs}ms`
    );
  }
  ...
  private getDefaultLogger() {
    return {
      debug: (msg: string, ...args: any[]) =>
        console.debug(`[EventBatcher] ${msg}`, ...args),
      info: (msg: string, ...args: any[]) =>
        console.info(`[EventBatcher] ${msg}`, ...args),
      warn: (msg: string, ...args: any[]) =>
        console.warn(`[EventBatcher] ${msg}`, ...args),
      error: (msg: string, ...args: any[]) =>
        console.error(`[EventBatcher] ${msg}`, ...args),
    };
  }
  ...
  registerBatchCallback(callback: (batch: any[]) => Promise<void>): void {
    ...
    this.logger.debug(
      `Batch callback registered. Total callbacks: ${this.batchCallbacks.length}`
    );
  }
  ...
  addEvent(event: any): boolean {
    if (this.eventQueue.length >= this.config.maxQueueSize) {
      this.logger.warn(
        `Event queue at max capacity (${this.config.maxQueueSize}). Dropping oldest events.`
      );
  ...
  getMetrics(): BatchMetrics {
    return {
      ...
      averageProcessingTime: parseFloat(
        this.metrics.averageProcessingTime.toFixed(4)
      ),
    };
  }
