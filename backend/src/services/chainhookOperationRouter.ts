...
export class ChainhookOperationRouter {
  ...
  private getDefaultLogger() {
    return {
      debug: (msg: string, ...args: any[]) =>
        console.debug(`[OperationRouter] ${msg}`, ...args),
      info: (msg: string, ...args: any[]) =>
        console.info(`[OperationRouter] ${msg}`, ...args),
      warn: (msg: string, ...args: any[]) =>
        console.warn(`[OperationRouter] ${msg}`, ...args),
      error: (msg: string, ...args: any[]) =>
        console.error(`[OperationRouter] ${msg}`, ...args),
    };
  }
  ...
  async routeOperationBatch(
    ...
    this.logger.debug(
      `Batch: ${operations.length} operations → ${routedCount} routed`
    );
    return routedCount;
  }
  ...
  private matchesFilter(operation: any, filter?: OperationFilter): boolean {
    ...
    if (
      ...
      operation.contract_call?.contract !== filter.contractAddress
    ) {
      return false;
    }
  ...
  getMetrics(): RouteMetrics {
    return {
      ...
      averageRoutingTime: parseFloat(
        this.metrics.averageRoutingTime.toFixed(4)
      ),
    };
  }
