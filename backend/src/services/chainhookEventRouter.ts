...
export class ChainhookEventRouter {
  ...
  async routeEvent(event: any): Promise<void> {
    ...
    for (const rule of applicableRules) {
      try {
        ...
        this.logger.error(
          `Error executing handler for rule ${rule.id}:`,
          error as Error
        );
      }
    }
  ...
  private getApplicableRules(event: any): RoutingRule[] {
    return this.getActiveRoutingRules().filter((rule) =>
      this.matchesFilter(event, rule.filter)
    );
  }
  ...
  getEventRoutingHistory(
    ruleId?: string,
    limit: number = 100
  ): Array<{ ruleId: string; eventId: string; timestamp: Date }> {
  ...
  getRuleStatistics() {
    ...
    const totalHandlers = Array.from(this.routingRules.values()).reduce(
      (sum, rule) => sum + rule.handlers.length,
      0
    );
