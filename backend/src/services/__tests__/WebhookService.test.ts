...
describe('WebhookService', () => {
  ...
  describe('registerWebhook', () => {
    ...
    it('should reject invalid events', async () => {
      ...
      await expect(
        service.registerWebhook('https://example.com/webhook', [
          'invalid_event',
        ])
  ...
  describe('getActiveWebhooks', () => {
    it('should return active webhooks for specific event', async () => {
      ...
      await service.registerWebhook('https://example1.com/webhook', [
        'badge_mint',
      ]);
      await service.registerWebhook('https://example2.com/webhook', [
        'badge_verification',
      ]);

      const badgeMintWebhooks = await service.getActiveWebhooks('badge_mint');
      const badgeVerificationWebhooks = await service.getActiveWebhooks(
        'badge_verification'
      );
      ...
      expect(badgeVerificationWebhooks[0].url).toBe(
