...
import BadgeIssuanceAnalytics, {
  IssuanceMetrics,
} from './badgeIssuanceAnalytics';
import CommunityGrowthAnalytics, {
  CommunityGrowthMetrics,
} from './communityGrowthAnalytics';
import UserAcquisitionAnalytics, {
  UserAcquisitionMetrics,
} from './userAcquisitionAnalytics';
import BadgeDistributionAnalytics, {
  BadgeDistribution,
} from './badgeDistributionAnalytics';
...
export class AnalyticsAggregator extends EventEmitter {
  ...
  private cache: Map<string, { data: AggregatedAnalytics; timestamp: number }> =
    new Map();
      ...
      const aggregated: AggregatedAnalytics = {
        ...
        metrics: {
          ...
          engagementRate:
            users.totalUsers > 0
              ? Math.round(
                  (users.userEngagement.usersWithBadges / users.totalUsers) *
                    100
                )
              : 0,
        },
        ...
        metrics: {
          ...
          avgBadgesPerCommunity:
            aggregated.community.totalCommunities > 0
              ? Math.round(
                  aggregated.issuance.totalBadgesIssued /
                    aggregated.community.totalCommunities
                )
              : 0,
        },
        distribution: {
          ...
          badgesByLevel: Object.fromEntries(
            aggregated.distribution.byLevel.map((l) => [
              l.level.toString(),
              l.count,
            ])
          ),
          ...
          topCommunities: aggregated.community.mostActiveCommunities.slice(
            0,
            10
          ),
        },
