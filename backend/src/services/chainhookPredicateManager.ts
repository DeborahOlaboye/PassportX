...
export class ChainhookPredicateManager {
  ...
  getPredicatesByNetwork(network: string): Predicate[] {
    return Array.from(this.predicates.values()).filter(
      (p) => p.network === network
    );
  }
    ...
    try {
      ...
      for (const predicate of predicates) {
        try {
          ...
          this.logger.warn(
            `Failed to import predicate: ${predicate.uuid}`,
            error
          );
        }
      }
  ...
  compilePredicateFilters(predicateUuid: string): void {
    ...
    if (!predicate) {
      this.logger.warn(
        `Cannot compile filter for non-existent predicate: ${predicateUuid}`
      );
      return;
    }
