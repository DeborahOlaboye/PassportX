...
export const registerBadgeIssuance = async (payload: BadgeIssuancePayload) => {
  try {
    const template = await BadgeTemplate.findById(payload.templateId).populate(
      'community'
    );
...
export const revokeBadge = async (payload: BadgeRevocationPayload) => {
  try {
    ...
    if (
      !community.admins.includes(payload.issuerAddress) &&
      badge.issuer !== payload.issuerAddress
    ) {
