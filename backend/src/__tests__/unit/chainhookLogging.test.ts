...
describe('Chainhook Logging Services', () => {
  ...
  describe('ChainhookEventLogger', () => {
    ...
    it('should update event status', async () => {
      ...
      if (event) {
        const updated = await logger.updateEventStatus(
          event.eventId,
          'completed',
          {
            processingTime: 2000,
  ...
  describe('AlertService', () => {
    ...
    it('should detect performance anomalies', async () => {
      const isAnomaly = await alertService.checkPerformanceAnomaly(6000, 2, 10);

      expect(isAnomaly).toBe(true);
    });

    it('should detect connection anomalies', async () => {
      const isAnomaly = await alertService.checkConnectionAnomaly(false, 5);
