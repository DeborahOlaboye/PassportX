...
describe('ChainhookEventObserver', () => {
  ...
  beforeAll(() => {
    // generateAuthToken() now throws if CHAINHOOK_AUTH_TOKEN is not set
    process.env.CHAINHOOK_AUTH_TOKEN = 'test-auth-token-for-observer-tests';
  });
  ...
  describe('Subscription Management', () => {
    ...
    it('should filter subscriptions by event type', () => {
      ...
      const badgeMintSubs =
        subscriptionManager.getSubscriptionsByEventType('badge-mint');

      expect(badgeMintSubs).toHaveLength(2);
      expect(badgeMintSubs.every((s) => s.eventType === 'badge-mint')).toBe(
        true
      );
    });
  ...
  describe('Predicate Management', () => {
    ...
    it('should get all predicates', () => {
      ...
      predicateManager.createPredicate(
        'P1',
        'stacks-contract-call',
        'devnet',
        {},
    ...
    it('should filter predicates by network', () => {
      ...
      const mainnetPredicates =
        predicateManager.getPredicatesByNetwork('mainnet');

      expect(mainnetPredicates).toHaveLength(2);
      expect(mainnetPredicates.every((p) => p.network === 'mainnet')).toBe(
        true
      );
    });
