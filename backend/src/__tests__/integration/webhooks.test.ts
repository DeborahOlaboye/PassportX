...
describe('Webhook Routes', () => {
  ...
  describe('DELETE /api/webhooks/:id', () => {
    it('should delete webhook', async () => {
      ...
      await request(app).delete(`/api/webhooks/${webhookId}`).expect(200);

      const getResponse = await request(app).get('/api/webhooks').expect(200);
  ...
  describe('POST /api/webhooks/retry-failed', () => {
    it('should initiate retry process', async () => {
      ...
      expect(response.body.message).toBe(
        'Retry process initiated for failed webhooks'
      );
    });
  });
});
