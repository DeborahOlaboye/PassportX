...
export const REORG_CONFIG = {
  ...
  DATABASE_LOG_RETENTION_HOURS: parseInt(
    process.env.REORG_DATABASE_LOG_RETENTION_HOURS || '24'
  ),
  ...
  ALERT_THRESHOLDS: {
    ...
    FREQUENT_REORG_PER_HOUR: parseInt(
      process.env.REORG_FREQUENT_REORG_PER_HOUR || '5'
    ),

    // Large impact: transactions affected exceeding this value
    LARGE_IMPACT_TRANSACTIONS: parseInt(
      process.env.REORG_LARGE_IMPACT_TRANSACTIONS || '100'
    ),
  },
  ...
  SERVICE_CONFIG: {
    ...
    CLEANUP_INTERVAL_MS: parseInt(
      process.env.REORG_CLEANUP_INTERVAL_MS || '3600000'
    ), // 1 hour

    // Maximum rollback operations to keep in memory
    MAX_ROLLBACK_OPERATIONS: parseInt(
      process.env.REORG_MAX_ROLLBACK_OPERATIONS || '10000'
    ),
  },
};
