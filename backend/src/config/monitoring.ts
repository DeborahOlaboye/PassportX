...
export function validateMonitoringConfig(config: MonitoringConfig): boolean {
  ...
  if (
    config.alertThresholds?.performanceThreshold &&
    config.alertThresholds.performanceThreshold < 100
  ) {
  ...
  if (
    config.alertThresholds?.failureRateThreshold &&
    (config.alertThresholds.failureRateThreshold < 1 ||
      config.alertThresholds.failureRateThreshold > 100)
  ) {
    console.error(
      'Monitoring config: Failure rate threshold must be between 1-100'
    );
    return false;
  }
