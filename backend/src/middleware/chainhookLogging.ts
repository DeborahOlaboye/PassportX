...
export function getChainhookRequestLogsByPath(
  path: string,
  limit: number = 50
): RequestLog[] {
...
export function getChainhookRequestLogsByStatusCode(
  statusCode: number,
  limit: number = 50
): RequestLog[] {
  return requestLogs
...
export function getChainhookSlowRequests(
...
export function getChainhookRequestLogStatistics() {
  ...
  const averageDuration =
    requestLogs.length > 0
      ? requestLogs.reduce((sum, log) => sum + log.duration, 0) /
        requestLogs.length
      : 0;
  const maxDuration =
    requestLogs.length > 0
  ...
  const minDuration =
    requestLogs.length > 0
      ? Math.min(...requestLogs.map((log) => log.duration))
      : 0;
