...
export class ChainhookPerformanceMonitoring {
  ...
  printReport(): void {
    ...
    this.logger.info(
      `Bytes Processed: ${(this.bytesProcessed / 1024 / 1024).toFixed(2)}MB`
    );
    this.logger.info(
      `Avg Throughput: ${(this.bytesProcessed / 1024 / seconds).toFixed(2)}KB/s`
    );
    ...
    for (const [route, stats] of sorted) {
      ...
      this.logger.info(
        `  ${route}: ${stats.count} requests, avg ${avgTime.toFixed(2)}ms`
      );
    }
